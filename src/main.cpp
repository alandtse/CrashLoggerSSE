	...
	void InitializeLogging()
	{
		...
		*path += L".log";
		...
		try {
			...
			if (auto config = Settings::GetSingleton(); config) {
				const auto& debugConfig = config->GetDebug();
				log->set_level(debugConfig.logLevel);
				log->flush_on(debugConfig.flushLevel);
			}
		} catch (...) {
			loadError = true;
		}
